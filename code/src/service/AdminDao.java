...
public class AdminDao extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//学生操作
		case "query_all_":
			query_student(request, response);break;	
		...
		case "query_mycourse":
			query_mycourse(request, response);break;	
		case "insert_course":
		...
		//成绩操作
		case "query_grade":
			query_grade(request, response);break;	
		case "query_all_sc":
	...
	/*-------------------------------- 学生-----------------------------------*/
	//查询学生信息
		protected void query_student(HttpServletRequest request, HttpServletResponse response)
				throws ServletException, IOException {
			response.setContentType("text/html;charset=utf-8");
			HttpSession session = request.getSession();
			User user = (User) session.getAttribute("student");
			ArrayList<Student> results = new StudentDao().query_student(user.getUsername());
			PrintWriter out = response.getWriter();
			// 输出结果
			if (results != null) {
				out.write("<div class='all'>");
	...
	//查询我的课程
		protected void query_mycourse(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			response.setContentType("text/html;charset=utf-8");
			HttpSession session = request.getSession();
			User user = (User) session.getAttribute("student");
			ArrayList<Course> results = new CourseDao().query_my(user.getUsername());
			PrintWriter out = response.getWriter();
			// 输出结果
			if (results != null) {
	...
	/*-------------------------------- 成绩-----------------------------------*/
	// 查询成绩表
		protected void query_grade(HttpServletRequest request, HttpServletResponse response)
				throws ServletException, IOException {
			response.setContentType("text/html;charset=utf-8");
			HttpSession session = request.getSession();
			User user = (User) session.getAttribute("student");
			ArrayList<SC> results = new SCDao().query_all_sc(user.getUsername());
			PrintWriter out = response.getWriter();
			// 输出结果
			if (results != null) {
				out.write("<div id='all_sc' class='all'>");
